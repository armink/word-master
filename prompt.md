...
1. **数据库表结构设计**
   - 输出：`wordbooks`、`words`、`quiz_sessions`、`quiz_answers` 表的字段定义和 TypeScript 类型
   - 确认重点：字段是否覆盖所有页面需要展示的数据

2. **UX 设计**（依赖表结构，数据边界明确后再设计）
   - 输出：页面流程图（Mermaid）、每个页面的信息架构、核心交互状态说明
   - 确认重点：测验答题卡片的状态流转是否符合预期

3. **项目脚手架搭建**（依赖 UX，按页面建好目录和路由）
   - 输出：`frontend/` + `backend/` 目录结构、依赖安装、Tailwind/路由/ESLint 配置

4. **后端 API 开发**（不接讯飞，先跑通本地逻辑）
   - 输出：单词本导入、测验流程、答题记录的 RESTful API

5. **前端页面开发**（依赖后端 API 接口定义）
   - 输出：各页面 React 组件，对接后端 API

6. **讯飞语音接入**（最后接入，需要真实 AppID/APIKey）
   - 输出：语音识别、TTS、AI 语义识别服务层实现